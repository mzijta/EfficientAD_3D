...
from common3d import get_autoencoder256, get_autoencoder256_nopad, get_pdn_small, get_pdn_medium, \
...
def get_argparse():
    ...
    parser.add_argument('-s', '--subdataset', default='embryos_checked_split_week11',
                        help='One of 15 sub-datasets of Mvtec AD or 5' +
                             'sub-datasets of Mvtec LOCO')
    parser.add_argument('-o', '--output_dir', default='output_3d_model')
    parser.add_argument('-m', '--model_size', default='small',
                        choices=['small', 'medium'])
    parser.add_argument('-w', '--weights', default='output/pretraining/small_256_out10000/teacher_small_final_state.pth')
    ...
    parser.add_argument('-t', '--train_steps', type=int, default=20000)
...
image_size = 128
topad = True
experiment = 'small_256_pad_out10000'
...
def main():
    ...
    test_set_bad = list_full_paths(os.path.join(test_set_path,'defect'))
    ...
    tqdm_obj = tqdm(range(config.train_steps), total=config.train_steps, miniters=50, maxinterval=float("inf"))
    for iteration,  image_penalty in zip(
        ...
        distance_st_np = distance_st.cpu().detach().numpy() #new
        d_hard = np.quantile(distance_st_np, q=0.999) #new
        d_hard = torch.from_numpy(np.asarray(d_hard)) #new
